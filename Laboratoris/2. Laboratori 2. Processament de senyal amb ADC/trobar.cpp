...
void update_threshold(float sample) {
    ...
    if (sample_count >= samples_for_threshold) {
        ...
        printf("Nou llindar calculat: %d\n", static_cast<int>(round(threshold*100)));
        printf("Nou max calculat: %d\n", static_cast<int>(round(max_value*100)));
        printf("Nou min calculat: %d\n", static_cast<int>(round(min_value*100)));
...
int main() {
    ...
    while (1) {
        ...
        // 5 entre 2^12 = 1,22 mV
        signal_value = heart_sensor.read();  // Valor normalitzat entre 0.0 i 1.0
        //printf("signal value: %d\n", static_cast<int>(signal_value*1000));
        //int signal_value_to_int = static_cast<int>(signal_value*1000);
        // Actualitza el llindar automàticament després de cert nombre de mostres
        update_threshold(signal_value*1000);
        sample_count++;
        ...
        if (signal_value > threshold && previous_value <= threshold) {
            ...
            if (elapsed_time > 0) {
                ...
                //printf("Freqüència cardíaca: %d BPM\n", beats_per_minute);
            }
        }
